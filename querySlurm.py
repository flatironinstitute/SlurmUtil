...
class SlurmCmdQuery:
    ...
    def sacct_getReport (criteria, days=3, output='JobID,JobName,AllocCPUS,State,ExitCode,User,NodeList,Start,End', skipJobStep=True, cluster="Iron"):
        ...
        field_str, sacct_rlt = SlurmCmdQuery.sacctCmd (criteria, output, cluster=cluster)
        ...
        for line in sacct_rlt:
            ...
            #JobIDRaw does not have _ +
            job        = dict(zip(keys, ff))
            # replace uid by uname
            if 'State' in output and job['State'].startswith('CANCELLED by '):
               uid             = job['State'].rsplit(' ', 1)[1]
               job['State']    = '{} ({})'.format(job['State'], MyTool.getUser(uid))
