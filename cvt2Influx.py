...
class InfluxDBClientWrapper(object):
    def __init__(self, hostname="scclin011.simonsfoundation.com"):
        self.influx_client = influxdb.InfluxDBClient('localhost', 8086, "yliu", "", "slurmdb1")
...
class HostDataFile(object):
    def __init__(self, dataDir, hostname):
        self.data_file = open(dataDir + '/%s_sm.p'%hostname, 'rb')
        self.activePids= []
        self.eof       = False
...
if __name__ == '__main__':
    ...
    #Usage: python cvt2Influx.py worker0000 /mnt/ceph/users/yliu/tmp/mqtMonTest
    node, dataDir = sys.argv[1:3]

    ifclient     = InfluxDBClientWrapper()
    dataFile     = HostDataFile(dataDir, node)
    dataFile.cvtFile (ifclient, dataDir, node)
