...
def get_column(line):
    """
    Returns column name as in "  `creation_time` bigint(20) unsigned NOT NULL,"
    """
    m = re.match(' *`(.*)` .*',line)
...
def main():
    ...
    try:
        ...
        for line in fileinput.input():
            ...
            elif is_create(line):  # Look for an CREATE TABLE statement and begin to parse column names
                createStat = True
            elif createStat:       # parse column names
                col = get_column(line)
                if col:
                   columns.append(col)
                else:
